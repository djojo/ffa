...
// smartchSendSubscriptionEmail(85,4);
// smartchSendGroupProgressionEmail(85,4,2);
// smartchSendCompleteBlendedEmail(85,4,2);
// smartchSendCompleteElearningEmail(85,4);
// smartchSendQCMEndModuleEmail(2,2);
smartchSendQCMEndModuleReminderEmail(2,2);
...
function smartchSendSubscriptionEmail($userid, $courseid) {
    ...
    $contentmail .= '<p>Nous vous invitons à vous connecter dès maintenant pour profiter du contenu de formation, qui sera accessible jusqu\'au <strong>'.userdate($groupsession->enddate, '%d/%m/%Y').'</strong>.</p>';
...
function smartchSendGroupProgressionEmail($userid, $groupid, $planningid) {
    ...
    $contentmail .= '<div>';
    $contentinside = "";
    $contentinside .= '<p>Votre face à face pédagogique est prévu pour le <strong>'.userdate($planning->startdate, '%d/%m/%Y').'</strong>.</p>';
...
function smartchSendCompleteBlendedEmail($userid, $groupid, $planningid) {
    ...
    $contentmail .= '<div>';
    $contentinside = "";
    $contentinside .= '<p>Votre face à face pédagogique est prévu pour le <strong>'.userdate($planning->enddate, '%d/%m/%Y').'</strong>.</p>';
    $contentinside .= "<p>Il semble que vous n'avez pas complété l'intégralité de vos contenus pré - formation. Si vous l'avez complété dans les dernières 24h, merci d'ignorer ce message.</p>";
    $contentinside .= "<p>Nous vous invitons à vous connecter sur <a href='".getConfigValueByKey("url_plateforme")."'>".getConfigValueByKey("url_plateforme")."</a> dès maintenant pour compléter votre contenu e-learning avant votre face à face pédagogique.</p>";
    $contentmail .= $contentinside;
    $contentmail .= '<div>';
    $contentmail .= '<a href="'.getConfigValueByKey("url_plateforme").'" style="background:#00315A;border-radius:20px;display:inline-block;text-align:center;margin:20px auto;color:white;padding:10px 20px;text-decoration:none;">Accéder à mon compte</a>';
    $contentmail .= '</div>';
...
function smartchSendCompleteElearningEmail($userid, $groupid) {
    ...
    $contentmail .= '<div>';
    $contentinside = "";
    $contentinside .= '<p>Votre formation '.$course->fullname.' est disponible dans votre espace Formation 360 Athle pendant 15 jours supplémentaires. Au <strong>'.userdate($groupsession->enddate + 86400, '%d/%m/%Y').'</strong>, elle ne sera plus disponible dans votre compte.</p>';
    $contentinside .= "<p>Il semble que vous n'avez pas complété l'intégralité du contenu de formation.</p>";
...
function smartchSendQCMEndModuleEmail($userid, $courseid) {
    ...
    $contentinside = "";
    $contentinside .= '<p>Votre formation '.$course->fullname.' touche à sa fin. Il est maintenant temps de remplir votre questionnaire de fin de formation.</p>';
    ...
    $contentmail .= $contentinside;
    $contentmail .= '<div>';
    $contentmail .= "Nous vous invitons à vous connecter sur <a href='".getConfigValueByKey("url_plateforme")."'>".getConfigValueByKey("url_plateforme")."</a> dès maintenant pour remplir ce questionnaire.";
    $contentmail .= '<a href="'.getConfigValueByKey("url_plateforme").'" style="background:#00315A;border-radius:20px;display:inline-block;text-align:center;margin:20px auto;color:white;padding:10px 20px;text-decoration:none;">Accéder à mon compte</a>';
    $contentmail .= '</div>';
...
function smartchSendQCMEndModuleReminderEmail($userid, $courseid) {
    ...
    $contentinside = '';
    $contentinside .= '<div>';
    $contentinside .= "<p>Il semble que vous n'ayez pas encore complété le questionnaire de fin de formation de la formation ".$course->fullname.". Si vous l'avez complété dans les dernières 24h, merci d'ignorer ce message.</p>";
    $contentinside .= "<p>Le questionnaire est encore jusqu'au <strong>".userdate(time() + 24*60*60*31, '%d/%m/%Y')."</strong>.</p>";
    $contentmail .= $contentinside;
